...
https://data.inpi.fr/entreprises/481385649?q=freexian

You can download a certificate of registration there.
...
subject=C=FR, ST=Auvergne-Rhône-Alpes, L=Sorbiers, O=Freexian, serialNumber=481 385 649, CN=Freexian, businessCategory=Private Organization, jurisdictionL=Saint-Étienne, jurisdictionST=Auvergne-Rhône-Alpes, jurisdictionC=FR
...
*******************************************************************************
https://salsa.debian.org/lts-team/packages/shim (branch bullseye/updates):

https://salsa.debian.org/lts-team/packages/shim/-/commits/bullseye/updates

*******************************************************************************
### What patches are being applied and why:
Mention all the external patches and build process modifications, which are used during your building process, that make your shim binary be the exact one that you posted as part of this application.
*******************************************************************************
We keep these patches that were already a part of shim 15.1 for Debian 11 bullseye,
as can be seen in the tags linked from https://github.com/rhboot/shim-review/issues/417

Note that these patches don't affect non-arm64 builds, and that we are not looking for
signing for arm64 binaries, but we need to provide (unsigned) builds of them.
...
* February 2025
  ...
  * CVE-2025-1125
*******************************************************************************
GRUB in Debian 11 bullseye contains all fixes up to and including October 2023. We are currently working on an update for the newer issues, and this shim requires that update as it blocks grub,4 SBAT level.
...
*******************************************************************************
We (as in Debian) have applied missing patches to the linux kernel, see e.g.

https://salsa.debian.org/kernel-team/linux/-/blob/74d5d1538f8847328413b401794ceadd1c26ce7a/debian/patches/series#L114 (debian/6.1/bullseye branch)

We also use ephemeral keys to ensure that old (vulnerable) modules can't be loaded with newer kernels.
...
continue running. E.g. since we're continuing support for Debian 11 bullseye,
...
*******************************************************************************
* ```shim_16.1-2~deb11u1_amd64.build```
* ```shim_16.1-2~deb11u1_i386.build```
...
*******************************************************************************
We've moved forwards from 15.8 to 16.1 and dropped all the patches we used to need as they're upstream.
We've made the build-time change to update revocations.

*******************************************************************************
### What is the SHA256 hash of your final shim binary?
*******************************************************************************
024bce23e3c7fda70ff4a83fa990f57f8feaae439ae95e1a727f33e23b64aef6  shimx64.efi
774c0c2c48a0549fd17dcd8fad3eff2889f18051ac74fae4a44089a99265bbc3  shimia32.efi
...
about this.
*******************************************************************************
yes, we embed a CA certificate
yes, it says that it is a CA

```
$ openssl x509 -inform der -noout -text -in Freexian_Secure_Boot_CA.der
[...]
        X509v3 extensions:
...
grub (unmodified from Debian 11 bullseye):
...
which will be blocked via SBAT with this shim. The next grub update before this shim is updated will include this shim and a new grub build with more fixes included:

```
sbat,1,SBAT Version,sbat,1,https://github.com/rhboot/shim/blob/main/SBAT.md
grub,5,Free Software Foundation,grub,2.06,https://www.gnu.org/software/grub/
grub.debian,5,Debian,grub2,2.06-3~deb11u7,https://tracker.debian.org/pkg/grub2
grub.debian11,1,Debian,grub2,2.06-3~deb11u7,https://tracker.debian.org/pkg/grub2
```

fwupd (unmodified from Debian 11 bullseye):
...
*******************************************************************************
The current list in bullseye before our planned update to correct the 2025 CVEs is this.
Note that we plan to drop ntfs and possibly jfs in that update.
...
*******************************************************************************
GRUB2 2.06, Debian version 2.06-3~deb11u6

*******************************************************************************
### If your shim launches any other components apart from your bootloader, please provide further details on what is launched.
Hint: The most common case here will be a firmware updater like fwupd.
*******************************************************************************
We have fwupd in Debian 11 bullseye.
...
*******************************************************************************
Debian 11 bullseye supports Linux 5.10 and Linux 6.1, but for Debian 11 ELTS by Freexian, we will support Linux 6.1 and 6.12.

Missing bits to enforce Secure Boot are applied, see e.g.:
